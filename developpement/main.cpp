...
void initMatrix(){
    ...
    impliciteVolMatrix[2][0] = 0.3;
    intermediateValueMatrix[2][0] = 0.3;
    localVolMatrix[2][0] = 0.3;
    ...
    for (int j = 1; j < NB_MATURITY+1; j++){
        impliciteVolMatrix[0][j] = j;
        intermediateValueMatrix[0][j] = j;
        localVolMatrix[0][j] = j;
    }
...
bool initializeLocalVol(string filename){
    ...
    try {
        if (findVolatilityData(filename)){
            ...
            throw "C5 : Matrice de volatilite implicite non calculee";
...
double getLocalVol(double strike, double maturity){
    ...
    if (strike < STRIKE_OD)
        throw "C5 : Valeur trop petite";
    else if (strike > STRIKE_2Y)
        throw "C5 : Valeur trop grande";
    else if (maturity < MATURITY_0D || maturity > MATURITY_2Y)
        throw "C5 : Maturite erronee";
    else {
